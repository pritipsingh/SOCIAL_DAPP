...
const client = ipfsHttpClient({ host: 'ipfs.io' })
...
const Home = ({account, contract}) => {
  const [posts, setPosts] = useState('')
    ...
    //if they do set profile to true
      const balance = await contract.balanceOf(account);
      if( balance > 0 ) setHasProfile(true);
      //get all posts
