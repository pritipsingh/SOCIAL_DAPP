...
const client = ipfsHttpClient({host: 'nftstorage.link',
    setTimeout: 1000000,
})
...
const uploadToIPFS = async (event) => {
  ...
  if (typeof file !== 'undefined') {
      try {
        ...
        //   setAvatar(client.cat(`https://ipfs.io/ipfs/${result.path}`))
      } catch (error) {
...
if (loading) return (
  ...
  return (
    ...
    {profile ? (<div className="mb-3"><h3 className="mb-3">{profile.username}</h3>
        <img className="mb-3" alt="avatar" style={{ width: '400px' }} src={profile.avatar} /></div>)
        :
        <h4 className="mb-4">Become SOCIAL today and find your friends...</h4>
            ...
            <div className="content mx-auto">
                <Row className="g-4">
                    ...
                    <div className="d-grid px-0">
                        <Button onClick={mintProfile} variant="dark" size="lg">
                            Mint NFT Profile
                        </Button>
                    </div>
                </Row>
